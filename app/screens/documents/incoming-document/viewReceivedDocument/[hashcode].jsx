...
import DocumentReceiver from "../../../../../components/document/DocumentReceiver";
...
const ViewIncomingDocument = () => {
  ...
  const { data, refetch } = axios.get({
    key: ["incoming-documents", hashcode],
    url: "incoming-documents/" + hashcode,
  });

  return (
    <View className={`flex flex-1`}>
      <StackHeader headerTitle={"Incoming Document - Received"} />
      <ScrollView className={`flex-1`}>
        <View className="p-4 bg-slate-10 h-full">
          ...
          >
            <DocumentReceiver
              name={data?.receiver[0]?.receiver_name}
              recievedDate={data?.receiver[0]?.received_date}
            />
            ...
            <View>
              ...
              <Text className={`text-xs`} style={{ fontFamily: "Roboto" }}>
                Advance Copy: <Text>{data?.advance_copy}</Text>
              </Text>
              <Text className={`text-xs`} style={{ fontFamily: "Roboto" }}>
                Advance Copy Info: <Text>{data?.advance_info}</Text>
              </Text>

              <Text className={`text-xs`} style={{ fontFamily: "Roboto" }}>
                Document Action: <Text>{data?.document_action?.name}</Text>
              </Text>
            </View>
